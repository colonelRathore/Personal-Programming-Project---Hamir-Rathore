...
def DisplayProvinceOwner(roll_index, player_names, num_players, big_prov_list, provinces):
    prov_choice = input("Choose a province you would like to find the owner of: ")
    while prov_choice not in provinces:
        print("Please select a province that has been selected!")
        prov_choice = input()
    while prov_choice not in big_prov_list:
        print("This province is not owned!")
        print("If you would like to select a different province, please click r.")
        inp = input()
        if inp == "r":
            prov_choice = input("Please select a province: ")
    for province in big_prov_list:
        if province == province:
            own = num_players[roll_index]
